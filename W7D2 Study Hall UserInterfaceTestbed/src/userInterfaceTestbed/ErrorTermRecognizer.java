...
public class ErrorTermRecognizer {
	...
	public static String checkMeasureValue(String input) {
		...
		case 4:
			...
			if (currentCharNdx<input.length()) {
				measuredValueErrorMessage = "This character must be an \"E\", an \"e\".";
				return measuredValueErrorMessage + displayInput(input, currentCharNdx);
			}
		...
		case 7:
			// States 7 is similar to states 3 and 9, but it is a final state, so it must be
		...
		case 8:
			...
			measuredValueIndexofError = currentCharNdx;		// Copy the index of the current character;
			measuredValueErrorMessage = "The first character must be a decimal point.";
			return "The first character must be a digit or a decimal point.";
		default:
			return "";
		}
